function [A_peak, I_peak, peak_fitted, pars_fitted] = mrs_fitPeak_voigt( spectrum, peak_range, no_disp)    
...
function  [peak_fitted,pars_fitted] = fitPeak(data, x, no_disp)
    ...
    par_initials=[(data(1)+data(end))/2, x(index), 2.5, A_ini, 2.5]; 
        
    [peak_fitted, pars_fitted] = mrs_voigtFit(par_initials, data, x);
