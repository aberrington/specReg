...
%    do_single   read data in single precision        (false; true if >1GB)
...
if ~isempty(regexpi(fname,'\.h5$'))
    [data,header] = read_archive(fname,verb,do_single);
...
if (exist('GERecon','file') && use_ox)
    ...
    [data,header] = read_pfile(fname,do_single,verb);
else
    if verb
