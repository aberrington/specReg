...
% do_single  d in single precision                    (false; true if >1GB)
...
if isempty(do_single)
    dsize = 16*nframes*frame_size*nphases*nechoes*nslices*nreceivers;
    if dsize>1d9
        do_single = true;
    else
        do_single = false;
...
if nargout>1
    h = archive2header(archive,verb>2);
...
%% check data
if isempty(d),       warning('data is empty'); end
if any(isnan(d(:)))
